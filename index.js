...
exports.converter = function (color) {
    ...
    var rgbCode = void 0; //if user input is RGB code, use this variable
    var hexCode = void 0; //if user input is Hex code, use this one
    ...
    if (color.includes(',')) {
        //if comma or space is found, assign it to rgbCode as an array
        rgbCode = color.split(',');
    } else if (color.includes(' ')) {
        rgbCode = color.split(' ');
    } else {
        //else assign it to hexCode
        hexCode = color;
    }

    if (rgbCode) {
        rgbCode = rgbCode.map(function (el) {
            return el.trim();
        }); //every element is trimmed in case there are more spaces around it

        if (rgbCode.length > 3) {
            //there may be blank elements in the array due to the split, so filter the array to remove them
            rgbCode = rgbCode.filter(function (el) {
                return el.length > 0;
            });
        ...
        rgbCode = rgbCode.map(function (el) {
            ...
            return el < 256;
        }); //convert each string into a number and check if it is less than 256
        ...
        }); //convert each number into a hexadecimal string
        var hexResult = '#' + rgbCode.join(''); //join the hex string elements in the array to create a Hex code
...
exports.generateRandomRGB = function () {
    var rgb = 'rgb(' + [Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256)].join(', ') + ')';
    return rgb;
};
...
exports.testConverter = function (testNum) {
    ...
    while (counter < testNum) {
        var randomHex = exports.generateRandomHex();
        var randomRGB = exports.generateRandomRGB();

        var test1 = randomHex === exports.converter(exports.converter(randomHex));
        var test2 = randomRGB === exports.converter(exports.converter(randomRGB));
